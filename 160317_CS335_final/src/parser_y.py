...
def p_array_type(p):
  ...
  try:
      ...
      # print("value       -->",p[2].valueList)
      scopeDict[currScope].updateArgList(p[-1].idList[0],'size',p[2].valueList[0])
      pass
  except Exception as e:
...
def p_identifier_list(p):
    ...
    try:
        ...
        if checkId(p[1], "*"):
            ...
            # print ("identifiers list",p[0].placeList)
...
def p_expr_list(p):
    ...
    try:
        ...
        # print("expressionsList meeeeeeeee",p[1].extra)
        if 'code' not in p[1].extra:
            print("yes",p[1].idList)
            p[0].extra['code'] = ['None']        
        else:
            print("no")
            p[0].extra['code'] +=p[1].extra['code']
        pass
    except Exception as e:
...
def p_expr_rep(p):
    ...
    try:
        if len(p) == 4:
            ...
            if 'code' in p[3].extra:
                p[0].extra['code'] +=p[3].extra['code']
            else:
                p[0].extra['code'] = ['None']
        else:
            p[0] = p[1]
    ...
    try:
        if p[2] == '=':
            ...
            if len(p[3].placeList) == 0:
                p[0] = p[1]
                scope = findScope(p[1].idList[0])
                print("okkkkkkk",p[2].typeList)
                if(p[2].typeList[0][0]=='*'):
                    print("yessssssssssss")
                    arrlen = findInfo(p[1].idList[0])['size'] *4
                    print("length---->",arrlen)
                    newtmp = newTemp()
                    base_add = newTemp()
                    p[0].code.append([newtmp,'','=',arrlen])
                    p[0].code.append(['array','',base_add,newtmp])
                    scopeDict[scope].updateArgList(p[1].idList[0], 'base_add', base_add)
                ...
                for x in range(len(p[1].idList)):
                    ...
                    if p[2].typeList[0].startswith('type'):
                        # print("ddddddddddd")
                        var_name = p[1].idList[x]
                        print(var_name)
                        struct_name = p[2].typeList[0][4:]
                        print(struct_name)
                        info = findInfo(struct_name)
                        print("info ---",info)
                        newScopeTable = info['child']
                        for n in newScopeTable.table:
                            if newScopeTable.table[n]['type'] != 'temporary':
                                scopeDict[currScope].insert(var_name+'.'+n , newScopeTable.table[n]['type'])
                                newtemp = newTemp()
                                scopeDict[currScope].updateArgList(var_name+'.'+n , 'place', newtemp)
                            # print("newScopeTable.table[n]['type'] ----->>>",newScopeTable.table[n]['type'])
                return
...
def p_func(p):
    ...
    try:
        ...
        tmp1 = newTemp()
        tmp2 = newTemp()
        p[0].code = [['push','','',tmp2],['push','','',tmp1]] + p[0].code
        temp_code = []
        temp_list = []
        for x in range(len(p[1].idList)):
            if p[1].typeList[x].startswith('type'):
                var_name = p[1].idList[x]
                print("kkkkkkkkkkkkkkkkkkkkiiiiiiiiiippuuuu",var_name)
                struct_name = p[1].typeList[x][4:]
                print("hhhhhhhhhhhhhhhhhhhhhhhpuuuuuuuuuu",struct_name)
                info = findInfo(struct_name)
                print("info ---",info)
                newScopeTable = info['child']
                print(info['child'].table)
                for n in newScopeTable.table:
                    if newScopeTable.table[n]['type'] != 'temporary':
                        print("in for ---")
                        print("n",n)
                        val = var_name+'.'+n
                        print("val---",val)
                        scopeDict[currScope].insert(val,newScopeTable.table[n]['type'])
                        # scp = findScope(val)
                        # print("scp-------ppuuuuuuu",scp)
                        # value = scopeDict[scp].table[val]['place']
                        newtemp = newTemp()
                        scopeDict[currScope].updateArgList(val,'place',newtemp)
                        # newtemp += 'f'
                        # temp_list.append(newtemp)
                        # print("value-------",value)
                        temp_code.append(['push', " ",  " ",newtemp ])
            
            else:
                temp_code = [['push', '', '', p[1].placeList[x]]]
        print("cs2",currScope,scopeDict[currScope].table)
        p[0].code = temp_code + p[0].code
        # print("in push --")
        print("Table function:")
        print(scopeDict[currScope].table)
        ...
        for x in range(len(p[1].idList)):
            if p[1].typeList[x].startswith('type'):
                var_name = p[1].idList[x]
                print("kkkkkkkkkkkkkkkkkkkkiiiiiiiiii",var_name)
                struct_name = p[1].typeList[x][4:]
                print("hhhhhhhhhhhhhhhhhhhhhhh",struct_name)
                info = findInfo(struct_name)
                print("info ---",info)
                newScopeTable = info['child']
                for n in newScopeTable.table:
                    if newScopeTable.table[n]['type'] != 'temporary':
                        ...
                        temp_code.append(['pop', " ",0, scopeDict[currScope].table[val]['place']])
                        i = i+1
            else:
                info = findInfo(p[1].idList[x])
                print ("poppppp",p[1].placeList)
                temp_code = [['pop', '', len(p[1].idList) - x +1, p[1].placeList[x]]]

        print("Table function:")
        print(scopeDict[currScope].table)
        p[0].code = temp_code + p[0].code
        p[0].code = [['pop','',0,tmp1],['pop','',1,tmp2]] + p[0].code
...
def p_prim_expr(p):
    ...
    try:
        ...
        # First thing is to check if IDENTIFIER has been declared before - struct
            ...
            newPlace3 = newTemp()
            p[0].code.append([newPlace3, p[3].placeList[0], 'x', newPlace4])

            newPlace = newTemp()
            p[0].code.append([newPlace, base_add, '+', newPlace3])

            newPlace2 = newTemp()
            # p[0].code.append([ 'store','',newPlace2, newPlace])
            p[0].extra['code'] = [[ 'store','', newPlace,newPlace2],[ 'load','',newPlace2, newPlace]]
            # print("in PrimaryExpr ",p[0].extra)
            ...
            if len(p[3].placeList):
                # print("kkkkkkkkkkkkkkkkkkkggggggg",p[3].placeList,len(p[3].idList))

                for x in range(len(p[3].idList)):
                    # print("kkkkkkkkkkkkkkkkkkkk",p[3].typeList[x])

                    if p[3].typeList[x].startswith('type'):
                        var_name = p[3].idList[x]
                        # print("kkkkkkkkkkkkkkkkkkkkiiiiiiiiii",var_name)
                        struct_name = p[3].typeList[x][4:]
                        # print("hhhhhhhhhhhhhhhhhhhhhhh",struct_name)
                        info = findInfo(struct_name)
                        # print("info ---",info)
                        newScopeTable = info['child']
                        for n in newScopeTable.table:
                            # print ("omg",newScopeTable.table[n]['place'])
                            if newScopeTable.table[n]['type'] != 'temporary':
                                # print("in for ---")
                                # print("n",n)
                                val = var_name+'.'+n
                                scp = findInfo(val)
                                print("scp-------",val,scp['place'])
                                # value = scopeDict[scp].table[val]['place']
                                # print("value-------",value)
                                p[0].code.append(['push', " ",  " ",scp['place']])
                        # print("out for4566 ---")
                    else:
            ...
            print ("gogo",p[1].placeList[0],p[2].placeList[0])
            if len(p) == 3 and 'struct' in p[2].extra:
                print("nono" , p[1].placeList[0],p[2].placeList[0])
                infom = findInfo(p[2].idList[0])
                print("imm",infom['place'])
                p[0] = p[1]
                p[0].placeList = infom['place']
                p[0].typeList = p[2].typeList
                p[0].idList = p[2].idList
                print("sti",p[0].placeList,p[0].typeList,p[0].idList)
            elif not len(p[2].placeList):
                p[0] = Node()
            else:
                p[0] = p[1]
            ...
            print("aft",p[0].placeList,p[1].typeList,p[2].idList)
        pass
    except Exception as e:
...
def p_expr(p):
    ...
    try:
        if len(p) == 4:
            # print("fin",p[1].placeList,p[0].typeList,p[0].idList)
            # print("fin",p[3].placeList,p[3].typeList,p[3].idList)
            p[0] = Node()
            print("hey in expressions")
            if 'code' in p[1].extra and p[1].extra['code'][0] != 'None':
                p[1].code.append(p[1].extra['code'][1])
            if 'code' in p[3].extra and p[3].extra['code'][0] != 'None':
                p[3].code.append(p[3].extra['code'][1])
            p[0].code += p[1].code
            p[0].code +=p[3].code
            print(p[0].code)
            newPlace = newTemp()
            ...
            # Start -----------------------------------------------------------
            if p[2] == '+':
                print("DC",p[1].idList,p[1].typeList,p[1].placeList)
                infom = findInfo(p[1].idList[0])
                # print("yoyo",p[1].idList[0],infom) 
                if p[1].typeList[0]=='int' and p[3].typeList[0]=='int':
                    ...
                    p[0].code.append([p[0].placeList[0],p[1].placeList[0],'+',p[3].placeList[0] ])
                ...
                if p[1].typeList[0]=='int' and p[3].typeList[0]=='int':
                    ...
                    if(len(p[1].valueList) and len(p[3].valueList)):
                        p[0].valueList.append(p[1].valueList[0] * p[3].valueList[0])
                    p[0].code.append([p[0].placeList[0],p[1].placeList[0],'x',p[3].placeList[0] ])
                    scopeDict[scope].updateArgList(p[0].placeList[0], 'value', p[3].valueList[0])
                    scopeDict[scope].updateArgList(p[0].placeList[0], 'type', "int")
                ...
                p[0].code.append([result, p[1].placeList[0], p[2], p[3].placeList[0] ])
                p[0].extra['result']=[result]
                scopeDict[scope].updateArgList(p[0].placeList[0], 'value', p[3].valueList[0])
                scopeDict[scope].updateArgList(p[0].placeList[0], 'type', "bool")
                print("condition ==",p[3].placeList[0])
                ...
                result = newTemp()
                p[0].code.append([result, newPlace,p[2],p[1].placeList[0], p[3].placeList[0] ])
                p[0].extra['result']=[result]

        else:
            p[0] = p[1]
            print("expr",p[0].placeList,p[0].typeList,p[0].idList)
        pass
    except Exception as e:
        print("WARNING:23:"+str(e))
        return
...
def p_unary_expr(p):
    ...
    try:
        if len(p) == 2:
       	    p[0] = p[1]
            print ("uni",p[0].placeList,p[0].idList,p[0].typeList)
            # print("in unary ",p[0].extra)
        elif p[1] == "!":
            p[0] = p[2]
            newPlace = newTemp()
...
def p_print_stmt(p):
    ...
    try:
        p[0] = p[2]
        print("======print stmt",p[0].extra)
        if 'code' in p[0].extra and p[0].extra['code'][0] != 'None':
            p[0].code.append(p[0].extra['code'][1])
        # print("type list here fro print ----->",p[2].typeList[0])
        if(p[2].typeList[0] == "int"):
            p[0].code.append(['printint', ' ', ' ', p[2].placeList[0]]) # printint hard coded. change it later 
        else:
            p[0].code.append(['printstr', ' ', ' ', p[2].placeList[0]])
        pass
    except Exception as e:
...
def p_inc_dec(p):
  ...
  try:
      ...
      newtemp = newTemp()
      sometmp = newTemp()
      p[0].code.append([sometmp,'','=',1])
      p[0].code.append([p[1].placeList[0],p[1].placeList[0],p[2][0],sometmp])
...
def p_assignment(p):
  ...
  try:
      ...
      p[0] = Node()
      # print("p11111",p[1].extra)
      # print("p33333",p[3].extra)
      p[0].code = p[1].code
      p[0].code += p[3].code
      
      # print("fffffffffffffff",p[1].code,"-------------",p[3].code)
      for x in range(len(p[1].placeList)):
          ...
          print("-=-------",typeinfo,len(typeinfo),p[1].idList[x])
          if len(typeinfo)>4 and typeinfo[:4] == 'type':
            ...
            #       raise TypeError("types are not compatible")
              ...
              print("mmmmmmmmmmmmmmmmmmm",p[3].typeList[x],p[1].typeList[x])
              # print p[1].idList[x], p[3].typeList[x], p[1].typeList[x]
              ...
              if p[2][0] != '=':
                ...
                if 'code' in p[1].extra and p[1].extra['code'][0]!= 'None':
                    print("appending in p1")
                    p[0].code.append(p[1].extra['code'][0])
                if 'code' in p[3].extra and p[3].extra['code'][0]!= 'None':
                    print("appending in p3")
                    p[0].code.append(p[3].extra['code'][1])
                print("done w/o error")
              scopeDict[currScope].updateArgList(p[1].idList[x],'place',p[3].placeList[x])
              # print "seeeeeeeeeee ->",p[1].placeList[x] , p[2], p[3].placeList[x], p[3].valueList
              if len(p[3].valueList):
...
def p_if_statement(p):
  ...
  try:
      ...
      label1 = newLabel()
      newVar = newTemp()
      result = p[2].extra['result'][0]
      p[0].code += [[newVar, ' ', '=', result]]
      print("I have to change ",p[2].placeList[0])
      newVar2 = newTemp()
      p[0].code += [[newVar2, ' ', '=', '1']]
...
def p_for(p):
  ...
  try:
      ...
      if 'forInc' in p[3].extra:
          if 'result' in p[3].extra:
            print("result found ---------->",p[3].extra['result'][0])
          else:
            print("result NOt FounD")
          # It implies that ConditionBlockOpt -> ForClause
          print("For Inc tha ye to")
          ...
          label1 = newLabel()
          label2 = newLabel()
          result = p[3].extra['result'][0]
          if 'result' in p[3].extra:
            print("result found ---------->",p[3].extra['result'][0])
          else:
            print("result NOt FounD")
          three_address_code_jump = [t1,'','=',result],[t2,'','=','1'],[t1,t2,'-',t1],['ifgoto','', t1, label2]
          p[0].code = [['label', ' ', ':', label1]]
          p[0].code += p[3].code
...
def p_forclause(p):
  ...
  try:
      ...
      if len(p[3].placeList) != 0:
        ...
        newVar2 = newTemp()
        result = p[3].extra['result'][0]
        p[0].code += [[newVar, ' ', '=', result],[newVar2, ' ', '=', '1'],[newVar,newVar2,'-',newVar],['ifgoto','', newVar, label2]]
      # p[0].code += p[5].code
