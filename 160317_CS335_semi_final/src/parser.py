...
def p_array_length(p):
  ...
  if p[1].typeList[0][3:] != 'int':
    print("Array Length must be integer.....Near Line no",line)
    return
  p[0] = p[1]
    ...
    try:
        ...
        # First thing is to check if IDENTIFIER has been declared before - struct
            ...
            if p[3].typeList[0][3:] != "int":
                pos = p.lexer.lexpos
                line = checkLineNo(pos,0)
                print ("ERROR:70:", "Index Must be integer type ...Near line no ",line)
                return
            ...
            if len(p[3].placeList) != expected_params:
				print "number of expected Parameters are",expected_params,"but you passed",len(p[3].placeList)," ...Near Line no ..",line
				return
            if p[-1] == '=':
